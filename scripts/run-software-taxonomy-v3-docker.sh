...
if HOST_ROOT=$(pwd -W 2>/dev/null); then :; else HOST_ROOT=$ROOT; fi
...
exec docker run --rm \
  ...
  -e SEED="${SEED:-20260916}" \
  -e CONFLICT_WINDOW="${CONFLICT_WINDOW:-80}" \
  -e STRICT_GATE="${STRICT_GATE:-1}" \
  -e OUT_DIR="${OUT_DIR:-}" \
  -v "$HOST_ROOT:/workspace" \
  -w /workspace \
  "$IMAGE" sh -c 'git config --global --add safe.directory /workspace && exec sh scripts/run-software-taxonomy-v3.sh'
