...
if [ "${ALLOW_DIRTY:-0}" != "1" ] && [ -n "$(git status --porcelain)" ]; then
  echo "Refusing to run software decision gate on a dirty worktree. Commit/stash changes or set ALLOW_DIRTY=1." >&2
  exit 2
fi
...
OUT_DIR=${OUT_DIR:-results/software-decision-v2/$SHORT_COMMIT-$STAMP}
...
run_case() {
  ...
  java -cp "$JAR" ru.itam.typing.realistic.AccuracyEvaluationCli \
    ...
    --conflict-window "$CONFLICT_WINDOW" --out "$case_dir/baseline.json" > "$case_dir/baseline.log"
  java -cp "$JAR" ru.itam.typing.realistic.AccuracyEvaluationCli \
    --data "$normalized" --truth "$hold_truth" --rules rules/software-conservative-v2.yaml \
    --conflict-window "$CONFLICT_WINDOW" --out "$case_dir/candidate.json" > "$case_dir/candidate.log"
}

echo "Software-only paired decision scenarios..."
...
{
  echo "protocol=software-decision-v2"
  echo "sourceCommit=$SOURCE_COMMIT"
  echo "seed=$SEED"
  echo "softwareCount=$SOFTWARE_COUNT"
  echo "conflictPriorityWindow=$CONFLICT_WINDOW"
  echo "baselineRules=rules/canonical-rules.yaml"
  echo "candidateRules=rules/software-conservative-v2.yaml"
...
if [ "$STRICT_GATE" = "1" ]; then
  python3 scripts/summarize-software-decision-v2.py "$OUT_DIR" --window "$CONFLICT_WINDOW" --strict
else
  python3 scripts/summarize-software-decision-v2.py "$OUT_DIR" --window "$CONFLICT_WINDOW"
fi
