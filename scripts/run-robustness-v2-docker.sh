...
exec docker run --rm \
  ...
  -e ROBUSTNESS_COUNT="${ROBUSTNESS_COUNT:-100000}" \
  -e DISTRIBUTION_COUNT="${DISTRIBUTION_COUNT:-50000}" \
  -e SEED="${SEED:-20260911}" \
  -e CONFLICT_WINDOW="${CONFLICT_WINDOW:-80}" \
  -e STRICT_GATE="${STRICT_GATE:-1}" \
  -e OUT_DIR="${OUT_DIR:-}" \
  -v "$HOST_ROOT:/workspace" \
  -w /workspace \
  "$IMAGE" sh -c 'git config --global --add safe.directory /workspace && exec sh scripts/run-robustness-v2.sh'
