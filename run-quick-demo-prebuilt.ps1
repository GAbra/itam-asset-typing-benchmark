...
$image = "itam-typing-demo:prebuilt"

Write-Host "[0/5] Preflight"
docker image inspect $image *> $null
if ($LASTEXITCODE -ne 0) {
    throw "Image '$image' is not loaded. First run: docker load -i .\itam-typing-demo-prebuilt.tar"
}
...
Invoke-Native { docker run --rm -v $mount -w /workspace $image verify --data data/generated/normalized-10000.jsonl --out benchmark-results/verify-10000.json } "Differential verification failed"

Write-Host "[4/5] Simple repeatable benchmark"
Invoke-Native { docker run --rm -v $mount -w /workspace $image benchmark --data data/generated/normalized-10000.jsonl --warmup 2 --runs 5 --batch 2000 --out benchmark-results/benchmark-10000.json } "Benchmark failed"
